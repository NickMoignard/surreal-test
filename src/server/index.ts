...
import { Server as HttpServer, IncomingMessage, OutgoingMessage } from 'node:http';
...
async function createServer(cfg: Config): Promise<{ start: () => Promise<void>; stop: () => Promise<void> }> {
  let started = false;
  let starting = false;
  let stopping = false;
  ...
  async function shutdown(): Promise<void> {
    const forceShutdownTimeout = setTimeout(async () => {
      ...
      try {
        ...
        logger.error('failed to close server after exceeding shutdown timeout', err);
      }
    }, SHUTDOWN_TIMEOUT);

    const onRequest = (_: IncomingMessage, outgoingMessage: OutgoingMessage) => {
